...
public final class Player extends Entity {
	...
	public Model getPlayerModel() {
		...
		for (int j1 = 0; j1 < 5; j1++)
			if (anIntArray1700[j1] != 0) {
				model.recolour(GameClient.anIntArrayArray1003[j1][0],
						GameClient.anIntArrayArray1003[j1][anIntArray1700[j1]]);
				if (j1 == 1)
					model.recolour(GameClient.anIntArray1204[0], GameClient.anIntArray1204[anIntArray1700[j1]]);
			}
	...
	public Model getRotatedModel() {
		...
		if (tranformIntoModel != null) {
			if (GameClient.loopCycle >= transformedTimer)
				tranformIntoModel = null;
			if (GameClient.loopCycle >= startTimeTransform && GameClient.loopCycle < transformedTimer) {
		...
		if (model_1 == null) {
			...
			for (int j3 = 0; j3 < 5; j3++)
				if (anIntArray1700[j3] != 0) {
					model_1.recolour(GameClient.anIntArrayArray1003[j3][0],
							GameClient.anIntArrayArray1003[j3][anIntArray1700[j3]]);
					if (j3 == 1)
						model_1.recolour(GameClient.anIntArray1204[0], GameClient.anIntArray1204[anIntArray1700[j3]]);
				}
	...
	public void updatePlayerAppearance(Stream stream) {
		...
		for (int partId = 0; partId < 12; partId++) {
			...
			if (partId == 8)
				GameClient.myHeadAndJaw[0] = equipment[partId] - 256;
			if (partId == 11)
				GameClient.myHeadAndJaw[1] = equipment[partId] - 256;
		...
		playerDonator = stream.readUnsignedWord();
