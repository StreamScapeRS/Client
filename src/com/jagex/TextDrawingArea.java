...
public final class TextDrawingArea extends DrawingArea {
	...
	public void drawCenteredText(int colour, int x, String text, int y, boolean center) {
		drawRegularText(text, x - getTextWidth(text) / 2, y, colour, center);
	}
	...
	public void drawChatInput(int i, int j, String s, int l, boolean flag) {
		drawRegularText(s, j, l, i, flag);
	}

	public void drawYellText(int color, int backgroundColour, String text, int yCoord, int xCoord, int alpha,
			int alphaBackground) {
		drawTransparentText(backgroundColour, text, yCoord + 1, 822, (xCoord - method384(text) / 2) + 1,
				alphaBackground);
	...
	public void drawRegularText(String text, int x, int y, int color, boolean shadowed) {
		aBoolean1499 = false;
		int l = x;
		if (text == null)
			return;
		...
		for (int i1 = 0; i1 < text.length(); i1++)
			if (text.charAt(i1) == '@' && i1 + 4 < text.length() && text.charAt(i1 + 4) == '@') {
				...
				if (c != ' ') {
					if (shadowed)
						method392(aByteArrayArray1491[c], x + anIntArray1494[c] + 1, y + anIntArray1495[c] + 1,
		...
		if (aBoolean1499)
			DrawingArea.drawLine(y + (int) (anInt1497 * 0.69999999999999996D), 0x800000, x - l, l);
	}
