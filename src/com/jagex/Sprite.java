...
public class Sprite extends DrawingArea {
	...
	public static Image getCutted(Image image, int x, int y, int width, int height) {
		try {
			int[] pixels = new int[(width - x) * (height - y)];
			PixelGrabber grabber = new PixelGrabber(image, x, y, width - x, height - y, pixels, 0, width - x);
			grabber.grabPixels();
			BufferedImage cutted = new BufferedImage(width - x, height - y, BufferedImage.TYPE_INT_ARGB);
			cutted.setRGB(0, 0, cutted.getWidth(), cutted.getHeight(), pixels, 0, cutted.getWidth());
			return cutted;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public Sprite(byte spriteData[]) {
		try {
			...
			pixelgrabber.grabPixels();
			image = null;
			setTransparency(255, 0, 255); // pink transparency
			setTransparency(255, 255, 255); // white transparency
	...
	public Sprite(byte spriteData[], boolean dummy) {
		try {
			Image image = Toolkit.getDefaultToolkit().createImage(spriteData);
			ImageIcon sprite = new ImageIcon(image);
			myWidth = sprite.getIconWidth();
	...
	public Sprite(String img) {
		try {
			Image image = Toolkit.getDefaultToolkit().getImage(signlink.findcachedir() + "" + img + ".png");
			if (image == null) {
				return;
			}
			ImageIcon sprite = new ImageIcon(image);
	...
	public Sprite(String img, int width, int height) {
		try {
			Image image;
			try {
				image = Toolkit.getDefaultToolkit().getImage(signlink.findcachedir() + "" + img + ".png");
			} catch (Exception e) {
				e.printStackTrace();
				return;
			}
	...
	public Sprite(String img, int clientWidth, int clientHeight, boolean yas) {
		try {
			Image image = Toolkit.getDefaultToolkit().getImage(signlink.findcachedir() + "" + img + ".png");
			if (image == null)
				return;
	...
	public void drawAdvancedSprite(int x, int y) {
		int k = 256;
	...
	public void drawHDSprite(int posX, int posY) {
		posX += drawOffsetX;
		posY += drawOffsetY;
	...
	public void drawRotated(int dimension, int rotation, int yPosArray[], int zoom, int xPosArray[], int basePosition,
