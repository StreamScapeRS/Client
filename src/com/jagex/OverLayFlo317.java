...
public class OverLayFlo317 {
	...
	private void parse(ByteBuffer byteBuffer) {
		for (;;) {
			...
			if (attributeId == 0)
				...
				groundColorOverlay = ((byteBuffer.get() & 0xff) << 16) + ((byteBuffer.get() & 0xff) << 8)
						+ (byteBuffer.get() & 0xff);
				...
				detailedColor = ((byteBuffer.get() & 0xff) << 16) + ((byteBuffer.get() & 0xff) << 8)
						+ (byteBuffer.get() & 0xff);
				if (GameClient.getOption("hd_tex"))
				...
				detailedColor13 = ((byteBuffer.get() & 0xff) << 16) + ((byteBuffer.get() & 0xff) << 8)
						+ (byteBuffer.get() & 0xff);
	...
	private void rgbhsl(int color) {
		...
		if (chroma_overlay < 1)
			chroma_overlay = 1;
		weighted_hue = (int) (hue * chroma_overlay);
