...
public class RSFontSystem extends DrawingArea {
	...
	public static String handleOldSyntax(String text) {
		...
		if (GameClient.getOption("smilies") && !text.contains(":store")) {
	...
	public void drawBaseStringMoveXY(String string, int drawX, int drawY, int[] xModifier, int[] yModifier) {
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			if (character == 60) {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								Sprite icon = SpriteLoader.sprites[1030 + iconId];
							...
							if (!GameClient.getOption("chat_effects")) {
				...
				if (startIndex == -1 || GameClient.getOption("chat_effects")) {
					...
					if (strikethroughColor != -1) {
						DrawingArea474.drawHorizontalLine(drawX, drawY + (int) (baseCharacterHeight * 0.7), i_109_,
								strikethroughColor);
					}
					if (underlineColor != -1) {
	...
	public void drawBasicString(String string, int drawX, int drawY) {
		drawTransparentString(string, drawX, drawY, 256);
	}

	public void drawTransparentString(String string, int drawX, int drawY, int transparency) {
		drawY -= baseCharacterHeight;
		int startIndex = -1;
	...
	public void drawTransparentString(String string, int drawX, int drawY, int color, int shadow, int trans) {
		if (string != null) {
			setColorAndShadow(color, shadow);
			drawTransparentString(string, drawX, drawY, trans);
		}
	}
