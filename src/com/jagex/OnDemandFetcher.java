...

{
	...
	public static void writeFDWord(DataOutputStream dos, int i) {
		try {
	...
	public void dumpMaps() {
		...
		for (int i = 0; i < landscapeIds.length; i++) {
			try {
				byte abyte[] = GameClient.getClient().cacheIndices[4].get(landscapeIds[i]);
				if (abyte == null) {
		...
		for (int i = 0; i < objectMapIds.length; i++) {
			try {
				byte abyte[] = GameClient.getClient().cacheIndices[4].get(objectMapIds[i]);
				if (abyte == null) {
	...
	public final OnDemandRequest getNextNode() {
		...
		int bufferLen = gzipDecompressor.decompress(OnDemandRequest.buffer, gzipInputBuffer);
		OnDemandRequest.buffer = new byte[bufferLen];
		System.arraycopy(gzipInputBuffer, 0, OnDemandRequest.buffer, 0, bufferLen);
		return OnDemandRequest;
	}
	...
	public final void run() {
		try {
			while (running) {
				...
				if (GameClient.loggedIn && socket != null && outputStream != null
	...
	String crcNames[] = { "model_crc", "anim_crc", "midi_crc", "map_crc", "image_crc", "texture_crc" };

	public final void start(CacheArchive streamLoader, GameClient client) {

		// LOAD CRCS
		for (int i = 0; i < crcNames.length; i++) {
			byte[] crc_file = streamLoader.getDataForName(crcNames[i]);
			int length = 0;
