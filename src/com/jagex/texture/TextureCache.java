...
public class TextureCache {
	...
	public static void initTextures(int count, OnDemandFetcher updateManager_) {
		ProceduralTexture.init(count, updateManager_);
	...
	public static int[] getTexturePixels(int textureId) {
		if (textureId == 0)
			textureId = 24;
		ProceduralTexture texture = ProceduralTexture.get(textureId);
		if (texture == null)
			return null;
		...
		if (textureTexelPoolPointer > 0) { // Freed texture data arrays
											// available
			texels = texelArrayPool[--textureTexelPoolPointer];
			texelArrayPool[textureTexelPoolPointer] = null;
		} else { // No freed texture data arrays available, recycle least used
					// texture's array
			int lastUsed = 0;
			int target = -1;
			for (int i = 0; i < loadedTextureCount; i++) {
		...
		MaterialType def = textureId >= 0 && textureId < MaterialType.textures.length ? MaterialType.textures[textureId]
				: null;
		...
		for (int texelPtr = 0; texelPtr < 16384; texelPtr++) {
			...
			if (textureId == 1 || textureId == 24)
				...
				texel = adjustBrightness(texel, 1.0F);
			else
				texel = adjustBrightness(texel, 0.7F);
			texel &= 0xf8f8ff;
	...
	private static int adjustBrightnessLinear(int rgb, int factor) {
		return ((((rgb >>> 16) * factor) & 0xff00) << 8) | ((((rgb >>> 8) & 0xff) * factor) & 0xff00)
				| (((rgb & 0xff) * factor) >> 8);
	}
