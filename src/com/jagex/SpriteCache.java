...
public class SpriteCache {
	...
	public static void fetchIfNeeded(int spriteId) {
		if (spriteCache[spriteId] != null)
			return;
		onDemandFetcher.requestFileData(GameClient.IMAGE_IDX - 1, spriteId);
	}
	...
	public static void load(GameClient c) {
		SpriteCache.c = c;
	...
	public static void preloadLowPriorityImages() {
		...
		loadSprite(31, c.loadingPleaseWait);// loadingPleaseWait = new
											// Sprite("loadingPleaseWait");
		loadSprite(32, c.reestablish);// reestablish = new
										// Sprite("reestablish");
		...
		loadSprite(47, c.magicAuto);// magicAuto = new
									// Sprite("Player/magicauto");
		/*
		 * for (int i = 0; i <= 4; i++) { loadSprite(48+i, c.LOGOUT[i]);//LOGOUT[i] =
		 * new Sprite("Frame/X " + i); }
		 */
		for (int i = 0; i <= 4; i++) {
			c.ADVISOR[i] = new Sprite("Gameframe/A " + i);
		}
		/* End custom sprites */
		// loadSprite(80, c.newMapBack);//newMapBack = new
		// Sprite("Frame/Mapback");
		for (int i4 = 0; i4 < 33; i4++)
			loadSprite(81 + i4, c.hitMark[i4]);// hitMark[i4] = new
												// Sprite("/Hitmarks/hit " +
												// i4);
		for (int i4 = 0; i4 < 6; i4++)
			loadSprite(114 + i4, c.hitIcon[i4]);// hitIcon[i4] = new
												// Sprite("/Hitmarks/icon " +
												// i4);
